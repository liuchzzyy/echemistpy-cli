...
class LanheCCSReader(BaseReader):
    ...
    DURATION_SECOND_COLUMNS: ClassVar[dict[str, str]] = {
        "StepDuration_ms": "StepDuration_s",
        "StepTime_ms": "StepTime_s",
        "TestTime_ms": "TestTime_s",
    }
    FORMATTED_DURATION_COLUMNS: ClassVar[set[str]] = {"StepDuration", "StepTime", "TestTime"}
    PREFERRED_RECORD_COLUMNS: ClassVar[list[str]] = [
        "Cycle",
        "Step",
        "Record",
        "WorkMode",
    ...
    def _create_dataset(self, result: CCSParseResult, mass_override_g: float | None = None) -> xr.Dataset:
        ...
        if mass_override_g is not None and mass_override_g > 0:
            ...
            records["SpeEnergy/mWh/g"] = records["Energy/uWh"].astype(float).map(lambda value: value / (mass_override_g * 1000.0))

        for ms_column, seconds_column in self.DURATION_SECOND_COLUMNS.items():
            if ms_column in records:
                records[seconds_column] = pd.to_numeric(records[ms_column], errors="coerce") / 1000.0

        systime_values = records.pop("SysTime") if "SysTime" in records else None
        records = records.drop(columns=[col for col in self.FORMATTED_DURATION_COLUMNS if col in records])
        records = self._ordered_records(records)
        ...
        ds.attrs["source_columns"] = [str(column) for column in result.records.columns]
    ...
    def _create_bundle_metadata(self, path: Path, result: CCSParseResult, mass_override_g: float | None = None) -> Metadata:
        ...
        metadata["active_material"] = parsed_metadata.active_material_text
        metadata["Cycle_Summary"] = self._frame_to_records(result.cycles)
        metadata["Step_Summary"] = self._frame_to_records(result.steps)
        metadata["Log_Info"] = self._frame_to_records(result.logs)

        active_material_mass = self.active_material_mass or self._format_mass_text(parsed_metadata.active_material_mass_g)
        if active_material_mass:
            metadata["active_material_mass"] = active_material_mass
        if mass_override_g is not None:
            metadata["active_material_mass_g"] = mass_override_g
...
class CCSParser:
    ...
    def _parse_metadata(
        ...
        return CCSMetadata(
            ...
            header_start_time=_datetime_from_unix_ms(header_start_ms),
            log_start_time=_datetime_from_unix_ms(log_start_ms),
            finish_time=None,
    ...
    def _parse_records(
        ...
        for page_offset in range(data_start, len(data), PAGE_SIZE):
            ...
            for index_in_page in range(valid):
                ...
                sys_time = _datetime_from_unix_ms(wall_start_ms + step_time_ms)
...
def _datetime_from_unix_ms(value: int | None) -> datetime | None:
    if value is None:
        return None
